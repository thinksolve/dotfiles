...
}:
{
  ...
  # home.activation.testActivationScript = lib.hm.dag.entryBefore ["linkGeneration"] #possible alternative
  home.activation.generateZshrc =
    let
      homeDir = "${config.home.homeDirectory}";
      outputFile = "${homeDir}/.dotfiles/zsh/.zshrc";
    in
    lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      $DRY_RUN_CMD cat > ${outputFile} << 'EOF'

      #------ this file generated by nix as specified in ~/.dotfiles/nix/darwin/home.nix ----------
      #------ all changes to this file are ephemeral between nix builds ---------
      export ZSH_CONFIG="${homeDir}/.dotfiles/zsh"
  ...
  home.activation.generateZshenv =
    let
      homeDir = "${config.home.homeDirectory}";
      outputFile = "${homeDir}/.dotfiles/zsh/.zshenv";
    in
    lib.hm.dag.entryBefore [ "linkGeneration" ] ''
      ...
      export PNPM_HOME=${homeDir}/Library/pnpm
      export NIX_CURRENT_SYSTEM_BIN=/run/current-system/sw/bin
      export NIX_CURRENT_USER_BIN=${homeDir}/.nix-profile/bin
      export HM_PROFILE="${homeDir}/.local/state/nix/profiles/home-manager"
      export HOMEBREW_BIN="/opt/homebrew/bin"

      export PATH="$HM_PROFILE/bin:$NIX_CURRENT_SYSTEM_BIN:$NIX_CURRENT_USER_BIN:${homeDir}/.local/bin:${homeDir}/.config/emacs/bin:${homeDir}/bin:/usr/local/bin:$PNPM_HOME:$HOMEBREW_BIN:$PATH"
      EOF
        $DRY_RUN_CMD chmod +w ${outputFile}
        run echo "Test activation script completed - check ${outputFile}"
