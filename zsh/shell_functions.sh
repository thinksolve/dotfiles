...
sg() {
  ...
  if [[ -d "$target" ]]; then
    rg_target="$target"
    file_cmd="find '$target' -type f"
  elif [[ -f "$target" ]]; then
    rg_target=$(dirname "$target")
    file_cmd="find '$rg_target' -type f"
  else
    # list of paths
    rg_target=$(echo "$target" | tr '\n' ' ')
    file_cmd="echo '$target' | tr ' ' '\n' | grep -v '^$'"
  fi

  fzf --disabled --ansi \
    --bind "change:reload:if [[ -n {q} ]]; then rg --color=always -n --smart-case {q} $rg_target 2>/dev/null; else $file_cmd; fi || true" \
    --preview 'if [[ {2} =~ ^[0-9]+$ ]]; then bat --color=always {1} --highlight-line {2}; else bat --color=always {}; fi' \
    --delimiter=':' \
    < <(eval "$file_cmd")
}
...
aa() {
  #short circuit if abbr doesnt exist ... so fzf doesnt run
  local abbr_list
  abbr_list=$(abbr list) || return 1

  local selected=$(echo "$abbr_list" | fzf --reverse | cut -d= -f1 | tr -d '"')
